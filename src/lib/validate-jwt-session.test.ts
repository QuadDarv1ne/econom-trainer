...
vi.mock('./session-cache', async () => {
  ...
  return {
    ...
    getPendingValidation: vi.fn((_userId: string, fetchFn: () => Promise<unknown>) => fetchFn()),
  };
});
...
describe('validateJwtSession', () => {
  ...
  it('returns token unchanged if userId is not a string', async () => {
    const token: Record<string, unknown> = { id: null, sessionHash: 'abc' };
  ...
  it('returns token unchanged if sessionHash is not a string', async () => {
    const token: Record<string, unknown> = { id: 'user123', sessionHash: null };
    const result = await validateJwtSession(token as never);
    expect(result).toBe(token);
    expect(sessionCache.getCachedSessionHash).not.toHaveBeenCalled();
  });
