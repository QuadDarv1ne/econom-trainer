...
export function calcEquilibrium(
  ...
  const eqQ =
    (demandIntercept + demandShift - (supplyIntercept + supplyShift)) / (demandSlope + supplySlope)
...
const PRACTICE_INTERCEPT_MIN = 40
const PRACTICE_INTERCEPT_MAX = 80
const PRACTICE_SLOPE_MIN = 0.5
const PRACTICE_SLOPE_MAX = 2
const PRACTICE_SUPPLY_INTERCEPT_MIN = 5
const PRACTICE_SUPPLY_INTERCEPT_MAX = 20
const PRACTICE_SUPPLY_SLOPE_MIN = 0.3
const PRACTICE_SUPPLY_SLOPE_MAX = 1.5
const PRACTICE_SHIFT_MIN = 5
const PRACTICE_SHIFT_MAX = 20
...
export function generatePracticeProblem(t: (key: string) => string): PracticeProblem {
  ...
  const a = Math.round(
    Math.random() * (PRACTICE_INTERCEPT_MAX - PRACTICE_INTERCEPT_MIN) + PRACTICE_INTERCEPT_MIN
  )
  const b =
    Math.round(
      (Math.random() * (PRACTICE_SLOPE_MAX - PRACTICE_SLOPE_MIN) + PRACTICE_SLOPE_MIN) * 10
    ) / 10
  const c = Math.round(
    Math.random() * (PRACTICE_SUPPLY_INTERCEPT_MAX - PRACTICE_SUPPLY_INTERCEPT_MIN) +
      PRACTICE_SUPPLY_INTERCEPT_MIN
  )
  const d =
    Math.round(
      (Math.random() * (PRACTICE_SUPPLY_SLOPE_MAX - PRACTICE_SUPPLY_SLOPE_MIN) +
        PRACTICE_SUPPLY_SLOPE_MIN) *
        10
    ) / 10
  let question: string
  let answer: number
  ...
  if (type === 'equilibrium') {
    const p = (a - c) / (b + d)
    question = t('supply-demand.practice.equilibrium')
      .replace('{a}', String(a))
      .replace('{b}', String(b))
      .replace('{c}', String(c))
      .replace('{d}', String(d))
    ...
    question = t('supply-demand.practice.elasticity')
      .replace('{a}', String(a))
      .replace('{b}', String(b))
      .replace('{p}', String(p))
    ...
    const shift = Math.round(
      Math.random() * (PRACTICE_SHIFT_MAX - PRACTICE_SHIFT_MIN) + PRACTICE_SHIFT_MIN
    )
    const eqQ1 = (a - c) / (b + d)
    const eqQ2 = (a + shift - c) / (b + d)
    const deltaQ = eqQ2 - eqQ1
    question = t('supply-demand.practice.shift')
      .replace('{a}', String(a))
      .replace('{b}', String(b))
      .replace('{c}', String(c))
      .replace('{d}', String(d))
...
export const SupplyDemand = memo(function SupplyDemand() {
  ...
  const awardXP = useCallback(() => {
    if (!hasEarnedXPRef.current) {
      ...
      addModuleInteraction({
        moduleId: 'supply-demand',
        action: 'interact',
        xpEarned: MODULE_XP['supply-demand'],
      })
    }
  }, [addModuleInteraction])
  ...
  const getEffectOnPrice = () => {
    if (demandShift > 0 && supplyShift < 0)
      return { text: t('supply-demand.effect.price.up'), variant: 'destructive' as const }
    if (demandShift < 0 && supplyShift > 0)
      return { text: t('supply-demand.effect.price.down'), variant: 'secondary' as const }
    if (demandShift > 0 || supplyShift < 0)
      return { text: t('supply-demand.effect.price.up.pressure'), variant: 'default' as const }
    if (demandShift < 0 || supplyShift > 0)
      return { text: t('supply-demand.effect.price.down.pressure'), variant: 'secondary' as const }
    return { text: t('supply-demand.effect.price.stable'), variant: 'outline' as const }
  }

  const getEffectOnQuantity = () => {
    if (demandShift > 0 && supplyShift > 0)
      return { text: t('supply-demand.effect.quantity.up'), variant: 'default' as const }
    if (demandShift < 0 && supplyShift < 0)
      return { text: t('supply-demand.effect.quantity.down'), variant: 'destructive' as const }
    if (demandShift !== 0 && supplyShift === 0)
      return { text: t('supply-demand.effect.quantity.change'), variant: 'secondary' as const }
        ...
        <TabsContent value="explore" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('supply-demand.description')}</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis
                      dataKey="quantity"
                      label={{
                        value: t('supply-demand.axis.quantity'),
                    ...
                    <YAxis
                      label={{
                        value: t('supply-demand.axis.price'),
                        angle: -90,
                        position: 'insideLeft',
                      }}
                      fontSize={12}
                    ...
                    <Tooltip
                      ...
                      formatter={(value: number, name: string) => [
                        value.toFixed(1),
                        name === 'demand'
                          ? t('supply-demand.tooltip.demand')
                          : t('supply-demand.tooltip.supply'),
                      ]}
                    ...
                    <Legend
                      formatter={(value) =>
                        value === 'demand'
                          ? t('supply-demand.legend.demand')
                          : t('supply-demand.legend.supply')
                      }
                    />
                    <ReferenceDot
              ...
              <div aria-live="polite" className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <div className="text-sm text-muted-foreground">
                    {t('supply-demand.summary.price')}
                  </div>
                  <div className="text-xl font-mono font-bold">{equilibrium.price.toFixed(1)}</div>
                </div>
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <div className="text-sm text-muted-foreground">
                    {t('supply-demand.summary.quantity')}
                  </div>
                  <div className="text-xl font-mono font-bold">
                    {equilibrium.quantity.toFixed(1)}
                  </div>
                </div>
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <div className="text-sm text-muted-foreground mb-1">
                    {t('supply-demand.summary.priceEffect')}
                  </div>
                  <Badge variant={getEffectOnPrice().variant}>{getEffectOnPrice().text}</Badge>
                </div>
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <div className="text-sm text-muted-foreground mb-1">
                    {t('supply-demand.summary.quantityEffect')}
                  </div>
                  <Badge variant={getEffectOnQuantity().variant}>
                    {getEffectOnQuantity().text}
                  </Badge>
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              {practiceResult === 'correct' && (
                <div className="flex items-center gap-2 text-green-600 text-sm">
                  ...
                  {t('supply-demand.practice.correct')} {t('supply-demand.practice.answer')}:{' '}
                  {practiceProblem.answer}
                </div>
              )}
              {practiceResult === 'incorrect' && (
                <div className="flex items-center gap-2 text-red-600 text-sm">
                  <XCircle className="h-4 w-4" />
                  {t('supply-demand.practice.incorrect')}{' '}
                  {t('supply-demand.practice.correctAnswer')}: {practiceProblem.answer}
                </div>
              )}
            </CardContent>
          </Card>
